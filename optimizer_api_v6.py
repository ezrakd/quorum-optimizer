...
# Computes what fraction of attribution MAIDs exist in our household universe.
# If only 11% are known, multiplier = 1/0.11 ≈ 9x (for every observed visit,
# ~8 more likely occurred but we can't see them).
# =============================================================================
def get_coverage_multiplier(cursor, table, agency_col, agency_id, advertiser_col=None,
                            advertiser_id=None, date_col=None, start_date=None, end_date=None):
    """Return coverage multiplier for a given agency (+ optional advertiser) from an attribution table.
    Returns (multiplier, coverage_pct, total_maids, known_maids).
    Falls back to 1.0 if computation fails."""
    try:
        where_clauses = [f"a.{agency_col} = %(agency_id)s"]
        params = {'agency_id': agency_id}
        if advertiser_col and advertiser_id:
            where_clauses.append(f"a.{advertiser_col} = %(advertiser_id)s")
            params['advertiser_id'] = advertiser_id
        if date_col and start_date and end_date:
            where_clauses.append(f"a.{date_col} BETWEEN %(start_date)s AND %(end_date)s")
            params['start_date'] = start_date
            params['end_date'] = end_date
        where_sql = ' AND '.join(where_clauses)
        cursor.execute(f"""
            SELECT COUNT(DISTINCT a.MAID) AS total_maids,
        ...
        """, params)
        row = cursor.fetchone()
        total_maids = int(row[0]) if row[0] else 0
        known_maids = int(row[1]) if row[1] else 0
        if known_maids > 0 and total_maids > 0:
            coverage_pct = known_maids / total_maids
            multiplier = 1.0 / coverage_pct
            return (round(multiplier, 2), round(coverage_pct * 100, 2), total_maids, known_maids)
        return (1.0, 0.0, total_maids, known_maids)
    except Exception:
        return (1.0, 0.0, 0, 0)
...
def enrich_web_visits_advertiser(cursor, agency_id, advertiser_id, start_date, end_date):
    ...
    try:
        cursor.execute("""
            SELECT COUNT(DISTINCT COALESCE(CAST(a.HOUSEHOLD_ID AS VARCHAR), a.MAID) || '|' || a.WEB_VISIT_DATE) as WEB_VISITS
            FROM QUORUMDB.DERIVED_TABLES.AD_TO_WEB_VISIT_ATTRIBUTION a
...
def enrich_web_visits_timeseries(cursor, agency_id, advertiser_id, start_date, end_date):
    ...
    try:
        cursor.execute("""
            SELECT a.WEB_VISIT_DATE, COUNT(DISTINCT COALESCE(CAST(a.HOUSEHOLD_ID AS VARCHAR), a.MAID)) as WEB_VISITS
            FROM QUORUMDB.DERIVED_TABLES.AD_TO_WEB_VISIT_ATTRIBUTION a
            WHERE a.AD_IMPRESSION_AGENCY_ID = %(agency_id)s
              AND a.AD_IMPRESSION_ADVERTISER_ID = %(advertiser_id)s
              AND a.WEB_VISIT_DATE BETWEEN %(start_date)s AND %(end_date)s
...
# =============================================================================
# STORE VISIT ENRICHMENT
# Reads from TWO sources:
#   1. WEB_TO_STORE_VISIT_ATTRIBUTION (derived) — web→store attribution (some agencies)
#   2. STORE_VISITS (base) — ad→store attribution (other agencies, e.g. Causal iQ)
# The two tables have almost no agency overlap, so we UNION them.
# =============================================================================
def enrich_store_visits_agency(cursor, start_date, end_date):
    """Return {agency_id: unique_store_visitor_count} from both store visit sources."""
    try:
        cursor.execute("""
            WITH combined AS (
                SELECT AGENCY_ID, COALESCE(CAST(HOUSEHOLD_ID AS VARCHAR), MAID) as VISITOR_ID
                FROM QUORUMDB.DERIVED_TABLES.WEB_TO_STORE_VISIT_ATTRIBUTION
                WHERE STORE_VISIT_DATE BETWEEN %(start_date)s AND %(end_date)s
                UNION ALL
                SELECT AGENCY_ID, MAID as VISITOR_ID
                FROM QUORUMDB.BASE_TABLES.STORE_VISITS
                WHERE STORE_VISIT_DATE BETWEEN %(start_date)s AND %(end_date)s
            )
            SELECT AGENCY_ID, COUNT(DISTINCT VISITOR_ID) as STORE_VISITS
            FROM combined
            GROUP BY AGENCY_ID
        """, {'start_date': start_date, 'end_date': end_date})
        return {int(r[0]): int(r[1]) for r in cursor.fetchall()}
...
def enrich_store_visits_advertiser(cursor, agency_id, advertiser_id, start_date, end_date):
    """Return unique store visitor count for a specific advertiser from both store visit sources."""
    try:
        cursor.execute("""
            WITH combined AS (
                SELECT COALESCE(CAST(HOUSEHOLD_ID AS VARCHAR), MAID) as VISITOR_ID
                FROM QUORUMDB.DERIVED_TABLES.WEB_TO_STORE_VISIT_ATTRIBUTION
                WHERE AGENCY_ID = %(agency_id)s
                  AND ADVERTISER_ID = %(advertiser_id)s
                  AND STORE_VISIT_DATE BETWEEN %(start_date)s AND %(end_date)s
                UNION ALL
                SELECT MAID as VISITOR_ID
                FROM QUORUMDB.BASE_TABLES.STORE_VISITS
                WHERE AGENCY_ID = %(agency_id)s
                  AND QUORUM_ADVERTISER_ID = %(advertiser_id)s
                  AND STORE_VISIT_DATE BETWEEN %(start_date)s AND %(end_date)s
            )
            SELECT COUNT(DISTINCT VISITOR_ID) as STORE_VISITS FROM combined
        """, {'agency_id': agency_id, 'advertiser_id': advertiser_id,
              'start_date': start_date, 'end_date': end_date})
        row = cursor.fetchone()
        return int(row[0]) if row else 0
...
def enrich_store_visits_timeseries(cursor, agency_id, advertiser_id, start_date, end_date):
    """Return {date_str: unique_store_visitor_count} for timeseries from both store visit sources."""
    try:
        cursor.execute("""
            WITH combined AS (
                SELECT STORE_VISIT_DATE, COALESCE(CAST(HOUSEHOLD_ID AS VARCHAR), MAID) as VISITOR_ID
                FROM QUORUMDB.DERIVED_TABLES.WEB_TO_STORE_VISIT_ATTRIBUTION
                WHERE AGENCY_ID = %(agency_id)s
                  AND ADVERTISER_ID = %(advertiser_id)s
                  AND STORE_VISIT_DATE BETWEEN %(start_date)s AND %(end_date)s
                UNION ALL
                SELECT STORE_VISIT_DATE, MAID as VISITOR_ID
                FROM QUORUMDB.BASE_TABLES.STORE_VISITS
                WHERE AGENCY_ID = %(agency_id)s
                  AND QUORUM_ADVERTISER_ID = %(advertiser_id)s
                  AND STORE_VISIT_DATE BETWEEN %(start_date)s AND %(end_date)s
            )
            SELECT STORE_VISIT_DATE, COUNT(DISTINCT VISITOR_ID) as STORE_VISITS
            FROM combined
...
def get_agencies():
    try:
        ...
        # Enrich ALL agencies with store visits from both sources
        # (WEB_TO_STORE_VISIT_ATTRIBUTION + STORE_VISITS base table)
...
def get_advertisers():
    try:
        ...
        # Enrich ALL advertisers with store visits from BOTH sources
        store_by_adv = {}
        try:
            cursor.execute("""
                WITH combined AS (
                    SELECT ADVERTISER_ID,
                           COALESCE(CAST(HOUSEHOLD_ID AS VARCHAR), MAID) || '|' || STORE_VISIT_DATE as VISIT_KEY
                    FROM QUORUMDB.DERIVED_TABLES.WEB_TO_STORE_VISIT_ATTRIBUTION
                    WHERE AGENCY_ID = %(agency_id)s
                      AND STORE_VISIT_DATE BETWEEN %(start_date)s AND %(end_date)s
                    UNION ALL
                    SELECT QUORUM_ADVERTISER_ID as ADVERTISER_ID,
                           MAID || '|' || STORE_VISIT_DATE as VISIT_KEY
                    FROM QUORUMDB.BASE_TABLES.STORE_VISITS
                    WHERE AGENCY_ID = %(agency_id)s
                      AND STORE_VISIT_DATE BETWEEN %(start_date)s AND %(end_date)s
                )
                SELECT ADVERTISER_ID, COUNT(DISTINCT VISIT_KEY) as STORE_VISITS
                FROM combined
